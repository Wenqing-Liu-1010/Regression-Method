...
transformed parameters {
  ...
  for(n in 1:N){
    mu_heads_up[n] = ((alpha) + gamma_j[map_j[n]] * sigma_gamma_j_mode) + ((theta) + gamma_k[map_k[n]]);
    mu_tails_up[n] = ((alpha) + gamma_j[map_j[n]] * sigma_gamma_j_mode) - ((theta) + gamma_k[map_k[n]]);
  }
}
model {
  ...
  target += momnorm_lpdf(sigma_gamma_j | 1) - log(0.5);

  target += normal_lpdf(gamma_k | 0, sigma_gamma_k);
  target += normal_lpdf(gamma_j | 0, sigma_gamma_j);
