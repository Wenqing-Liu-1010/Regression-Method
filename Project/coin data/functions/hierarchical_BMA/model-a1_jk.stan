...
transformed parameters {
  ...
  for(n in 1:N){
    mu_heads_up[n] = (logit(alpha) + gamma_j[map_j[n]] * sigma_gamma_j) + (logit(theta) + gamma_k[map_k[n]]);
    mu_tails_up[n] = (logit(alpha) + gamma_j[map_j[n]] * sigma_gamma_j) - (logit(theta) + gamma_k[map_k[n]]);
  }
}
model {
