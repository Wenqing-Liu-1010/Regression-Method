data {
  ...
  int L; // number of covariates
  matrix[N, L] x; // covariates
...
parameters {
  ...
  vector[L] beta;                 // covariates for the probability of same side (on logistic scale)
}
transformed parameters {
  ...
  for(n in 1:N){
    mu_heads_up[n] = (logit(alpha) + gamma_j[map_j[n]] * sigma_gamma_j) + (logit(theta) + gamma_k[map_k[n]] + x[n] * beta);
    mu_tails_up[n] = (logit(alpha) + gamma_j[map_j[n]] * sigma_gamma_j) - (logit(theta) + gamma_k[map_k[n]] + x[n] * beta);
  }
}
model {
